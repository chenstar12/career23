...
public int maxDepth(TreeNode root) { // 层序遍历（queue实现）
}
```
### 2. 平衡树
...
```java
public boolean isBalanced(TreeNode root) { // 暴力解, O(n**2)
        if (root == null) {
            return true;
        } else {
...
```
```java
public boolean isBalanced(TreeNode root) { // 剪枝：左右子树出现不平衡
    }
...
```java
    public TreeNode invertTree(TreeNode root) { //递归
        ...
        TreeNode temp = root.left;// 后面的操作会改变 left 指针，因此先保存下来
    ...
    public TreeNode invertTreeByQueue(TreeNode root) { // 层序遍历
    }

    private TreeNode invertTreeByStack(TreeNode root) { //深度优先遍历
    }
```
