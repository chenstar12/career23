...
题目：每个孩子都有一个满足度 grid，每个饼干都有一个大小 size，只有饼干大小大于等于孩子的满足度，该孩子才会满足。求最多可以满足的孩子数量。

1. 饼干应当尽量小并且又能满足该孩子，这样大饼干才能给满足度比较大的孩子。
2. 因为满足度最小的孩子最容易得到满足，所以先满足满足度最小的孩子。

每次分配饼干时选择一种当前最优的分配，但无法保证最后得到全局最优解。假设能得到全局最优解，并使用反证法进行证明，即假设存在一种比我们使用的贪心策略更优的最优策略。如果不存在这种最优策略，表示贪心策略就是最优策略，得到全局最优解。

证明：假设在某次选择中，贪心策略选择给当前满足度最小的孩子分配第 m 个饼干（满足该孩子的最小饼干）。假设存在一种最优策略，可以给该孩子分配第 n 个饼干，并且 m \< n。可以发现，经过这一轮分配，贪心策略分配后剩下的饼干一定有一个比最优策略来得大。因此在后续的分配中，贪心策略一定能满足更多的孩子。即不存在比贪心策略更优的策略，即贪心策略是最优策略。

```java
public int findContentChildren(int[] grid, int[] size) {
    if (grid == null || size == null) return 0;
    Arrays.sort(grid); // 排序：孩子的满足度
    Arrays.sort(size); // 排序：饼干size
    int gi = 0, si = 0;
...
题目描述：让一组区间不重叠所需移除的区间个数。

- 在每次选择中，区间的结尾最为重要，结尾越小，留给后面的区间的空间越大，后面能选择的区间个数也就越多。
...
题目描述：一个学生用两个分量 (h, k) 描述，h 表示身高，k 表示排在前面的有 k 个学生的身高 >= 他的身高。
...
题目描述：股票交易（买入和卖出），只进行一次交易，求最大收益。

记录当前最低价（作为买入价），将现在的价格作为售出价，判断是不是最大收益。
...
- 对于 [a, b, c, d]，如果 a \<= b \<= c \<= d ，则最大收益为 d - a。而 d - a = (d - c) + (c - b) + (b - a) 

- 因此每当访问prices[i] - prices[i-1] \> 0，则把 prices[i] - prices[i-1] 添加到收益中。
...
```
```python
def isSubsequence(self, s, t): # 双指针；贪心思想；
        p1, p2 = 0, 0
        while p1 < len(s) and p2 < len(t):
            if s[p1] == t[p2]: # 注：s是子序列
                p1 += 1
            p2 += 1
        return p1 == len(s)
```
