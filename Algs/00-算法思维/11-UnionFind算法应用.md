...
复习: Union-Find 解决图的动态连通性问题，算法本身不难，应用主要是看抽象问题的能力。

关键点有 3 个：

1、 `parent` 数组: 父节点，`parent` 数组实际存储着一个森林（若干棵多叉树）;

2、 `size` 数组: 每棵树的重量，目的是让 `union` 后树平衡，而不会退化成链表;

3、 `find` 函数中路径压缩，树的高度为常数， `union` 和 `connected` 降为 O(1)；

问：**既然有路径压缩，`size` 数组的重量平衡还需要吗**？因为路径压缩保证了树高为常数（不超过 3），即使树不平衡，高度也是常数，基本无影响。

答：论时间复杂度，确实不需要重量平衡也是 O(1)。但加上 `size` 数组辅助，效率略高一些，比如：
