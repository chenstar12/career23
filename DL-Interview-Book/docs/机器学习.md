...
[逻辑回归（Logistic Regression，LR）](https://en.wikipedia.org/wiki/Logistic_regression)也称"对数几率回归"，"逻辑斯谛回归";

**知识点提炼**

- **分类**，经典的二分类算法！
- 逻辑回归的过程：对分类问题建立**代价函数**，通过优化方法迭代求解出最优模型参数，验证模型的好坏。
-  logistic 主要应用于研究事件发生的概率。
- 逻辑回归的本质：极大似然估计
- 逻辑回归的激活函数：Sigmoid
- 逻辑回归的代价函数：交叉熵
...
- **核心是 [Sigmoid 函数](https://en.wikipedia.org/wiki/Sigmoid_function)**，可看成逻辑回归的激活函数。

逻辑回归：

<img src="imgs/DLIB-0001.png" alt="Logistic Regression.png" style="zoom:67%;" />

对数几率函数（Sigmoid）：

<img src="imgs/DLIB-0002.png" style="zoom:50%;" />

- 输出值在区间[0，1]，p(x) 可表示概率 p(y=1|x)，即当一个x发生时，y被分到1那一组的概率。

- 阈值通常是 0.5，当 y > 0.5 时，就将这个 x 归到 1 这一类，如果 y< 0.5 就将 x 归到 0 这一类。
...
**交叉熵作为代价函数**。代价函数](https://en.wikipedia.org/wiki/Loss_function)请参考[代价函数](http://www.cnblogs.com/Belter/p/6653773.html)。

交叉熵是对「出乎意料」（原文用suprise）的度量。神经元的目标是寻找最佳拟合函数 y = y(x)。但是取而代之计算交叉熵，衡量的是在知道 y 的真实值时的平均「出乎意料」程度。当输出是我们期望的值，我们的「出乎意料」程度比较低；当输出不是我们期望的，我们的「出乎意料」程度就比较高。
...
[支持向量机（supporr vector machine，SVM）](https://en.wikipedia.org/wiki/Support-vector_machine)是二类分类模型，该模型是定义在特征空间上的间隔最大的线性分类器。间隔最大使它有区别于感知机；支持向量机还包括核技巧，这使它成为实质上的非线性分类器。支持向量机的学习策略就是**间隔最大化**，可形式化为一个求解凸二次规划的最小化问题。
...
关于GBDT的理解来自论文greedy function approximation: a gradient boosting machine
...
Adaboost算法：将多个弱分类器（单层决策树）结合，使其成为一个强分类器。

Adaboost每次迭代只训练一个弱分类器，训练好的弱分类器将参与下一次迭代。即：在第N次迭代中，一共有N个弱分类器，其中N-1个是以前训练好的，其各种参数都不再改变，本次训练第N个分类器。其中弱分类器的关系是第N个弱分类器更可能分对前N-1个弱分类器**<u>分类错误的数据</u>**，最终分类输出要看这N个分类器的综合效果。
...
**XGBoost全名叫（eXtreme Gradient Boosting）极端梯度提升**；
...
机器学习三要素：模型、策略、算法。XGBoost算法以**集成思想**为基础。
...
　　kNN算法：样本在特征空间中k个最相邻的样本中的大多数属于某一个类别，则该样本也属于这个类别（分类决策只依据最邻近的k个样本的类别）

例子：如下图，绿色圆要被决定赋予哪个类，是红色三角形还是蓝色四方形？如果K=3，由于红色三角形所占比例为2/3，绿色圆将被赋予红色三角形那个类，如果K=5，由于蓝色四方形比例为3/5，因此绿色圆被赋予蓝色四方形类。
...
重新计算每个簇的质心是根据目标函数来的；在开始时要考虑距离度量和目标函数。

考虑欧几里得距离，使用误差平方和（Sum of the Squared Error, SSE）作为聚类的目标函数，两次运行K均值产生的两个不同的簇集，我们更喜欢SSE最小的那个。
...
Bagging：训练集是在原始集中**有放回**选取的，选出的各训练集之间相互独立.

Boosting：训练集一直不变，只是每个样例的权重发生变化（权值根据上一轮的分类结果调整）
...
Boosting：每个弱分类器都有相应权重，分类误差小的分类器有更大的权重.

4）并行计算：

Bagging：各个预测函数可以并行生成

Boosting：各个预测函数只能顺序生成（后一个模型需要之前模型的结果）
...
随机森林使用集成学习的**bagging**思想（集成学习主要分为bagging和boosting）

**Bagging（套袋法）**

（1）从原始样本集中使用Bootstraping方法随机抽取n个训练样本，共进行k轮抽取，得到k个训练集（训练集相互独立）
（2）k个训练集训练k个模型（如决策树，knn等）
（3）分类问题：**投票表决**产生分类结果；回归问题：k个模型预测结果的均值作为预测结果；

**Boosting（提升法）**

对训练集的每个样本赋权值wi；当某样本误分类，需加大对该样本权值。

迭代的过程中，每一步迭代都是一个弱分类器。我们需要用某种策略将其组合，作为最终模型。（例如AdaBoost给每个弱分类器一个权值，将其线性组合最为最终分类器。误差越小的弱分类器，权值越大）
...
决策树算法有ID3，C4.5，CART三种。思想都类似，只是采用了不同的指标。决策树模型的构建过程大致如下：
...
若A为空集（没有特征作为划分依据），则T为单节点树，并将D中实例数最大的类Ck作为该结点的类标记，返回T
否则，计算A中各特征的信息增益(ID3)/信息增益比(C4.5)，选择信息增益最大的特征Ag
...
与Bagging过程相似，随机森林的构建过程如下：

- 从原始训练集中用Bootstraping（有放回采样）出m个样本，共进行k次采样，生成k个训练集 ，训练k个决策树模型

- 对于单个决策树模型，假设训练样本特征数为n，则每次分裂时根据信息增益/信息增益比/基尼指数选择最好的特征进行分裂

- 每棵树都分裂下去，直到节点的所有训练样例属于同一类。决策树的分裂过程中不需要剪枝

- 将生成的多棵决策树组成随机森林。

  注：分类问题投票决定最终分类结果；回归由多棵树预测值的均值决定最终预测结果；
...
PCA（Principal Component Analysis）：用线性变换将原始数据变换为一组各维度线性无关的表示，用于提取数据的主要特征，常用于高维数据的降维。
...
ℓ1-norm和ℓ2-norm（L1正则化和L2正则化，或L1范数和L2范数）

损失函数的惩罚项：对损失函数的某些参数做限制。对于线性回归模型，用L1正则化的模型叫Lasso回归，用L2正则化的模型叫Ridge回归（岭回归）。

回归分析中w表示特征的系数，从上式可以看到正则化项是对系数做了处理（限制）。L1正则化和L2正则化的说明如下：
