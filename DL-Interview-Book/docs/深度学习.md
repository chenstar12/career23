...
- batchsize：批大小（批尺寸）。DL一般采用SGD训练，即每次训练在训练集中取batchsize个样本训练；

- iteration：中文译为迭代，1个iteration等于使用batchsize个样本训练一次；一个迭代 = 一个正向通过+一个反向通过

- epoch：1个epoch等于使用训练集中的全部样本训练一次；一个epoch = 所有训练样本的一个正向传递和一个反向传递
...
## CNN 所需计算力（flops）和参数（parameters）数量

对于一个卷积层（c为input channel, n为output channel）
...
mean pooling的前向传播就是取平均来做pooling，**反向传播**也就是把某个元素的梯度等分为n份分配给前一层，保证池化前后的梯度（残差）之和保持不变；
...
max pooling也要满足梯度之和不变的原则，max pooling的前向传播是把最大的值传递给后一层，其他像素的值直接舍弃。反向传播是把梯度直接传给前一层某一个像素，而其他像素不接受梯度，也就是为0。

- max pooling和mean pooling不同：需要记录下池化操作时到底哪个像素的值是最大，即max id（可以看caffe源码的pooling_layer.cpp），下面是caffe框架max pooling部分的源码
...
感受野（Receptive Field）：CNN每一层输出特征图（feature map）的像素点在**原始输入图片**上映射的区域大小。即：特征图的一个点对应**原始输入图片**上的区域；
...
因为DNN在非线性变换前的激活输入值（即x=WU+B，U是输入）随着网络层数加深，其分布逐渐发生偏移，导致反向传播时低层神经网络的梯度消失, 训练收敛慢；

BN规范化把每层输入值的分布拉回均值0方差1的标准正态分布（把越来越偏的分布拉回比较标准的分布），使得激活输入值落在非线性函数对输入比较敏感的区域（输入的小变化能导致损失函数较大的变化）
...
- BN适用于判别模型中，如图片分类。因为BN注重对每个batch进行归一化，从而保证数据分布的一致性，而判别模型的结果正是取决于数据整体分布。但是BN对batchsize的大小比较敏感，由于每次计算均值和方差是在一个batch上，所以如果$batchsize$太小，则计算的均值、方差不足以代表整个数据分布。
...
## 梯度下降法
梯度下降的**本质**：计算梯度，迭代更新权重参数使目标函数最小化。
...
小批量梯度下降是折中方案，选取训练集中一个小批量batch样本（如64,128等），训练过程更稳定；用批量训练也可以利用矩阵计算的优势。

### 随机梯度下降（SGD）

SGD是一个极端，针对训练集中的一个训练样本计算损失，又称**在线学习**。收敛速度快，但是目标函数值震荡，因为高频率的参数更新导致了高方差。

### 动量梯度下降法（Momentum）

引入一个指数加权平均的知识点。将之前的dW和db都联系起来，不再是每一次梯度都是独立的情况。
...
FCN将传统网络后面的全连接层换成了卷积层，网络输出不再是类别而是 heatmap；同时为了解决因为卷积和池化对图像尺寸的影响，提出使用上采样的方式恢复。
...
即反卷积（Deconvolution）。Caffe和Kera里叫Deconvolution，而tensorflow里叫conv_transpose。CS231n课中叫conv_transpose。

众所诸知，普通的池化会缩小图片的尺寸，比如VGG16 五次池化后图片被缩小了32倍。为了得到和原图等大的分割图，需要上采样/反卷积。

反卷积和卷积类似，都是相乘相加的运算。只不过后者是多对一，前者是一对多。而反卷积的前向和后向传播，只是颠倒卷积的前后向传播即可。
...
转置卷积（transposed Convolutions） == 反卷积（deconvolution） == 分数步长卷积（fractially straced convolutions）
