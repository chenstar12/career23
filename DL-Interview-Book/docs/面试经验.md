...
2. **能投提前批尽量不等正式批**。永远没有准备好的时候，有个60-80%的把握，其实就可以去面试了。把目标公司往年的面经拿出来，做模拟面试，60-80%的题目都能答出来。 

3. **能提前准备尽量别临时抱佛脚**。学校，比赛，论文，其实这些都是面试的主要参考，更早的让简历丰富，编程和算法基本功扎实。
...
2. 一定要刷题 
 	...
 	分类型进行刷题，对于某一类型的题目，先看一下该算法的核心思想。楼主在求职的过程中，遇到好多跟leetcode上类似的题目，刷题的目的不是为了碰见原题，而是为了熟练算法。

3. 机器学习的一些建议 

 	推荐西瓜书，统计学方法。熟悉一种深度学习框架。一定要实战，懂得更透彻。参加一些比赛，如kaggle,天池。
