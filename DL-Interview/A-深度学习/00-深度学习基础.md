...
- **欠拟合**：模型在**训练集**的**训练误差**大；
- **过拟合**：反映在**评价指标**上，模型在训练集上表现好，但是在测试集表现差（**泛化能力差**）；

### 降低过拟合

> 所有**减少测试误差**的策略统称**正则化方法**，可能会以增大训练误差为代价；

- **数据增强**
  - 图像：旋转、缩放
  - 生成新数据：用**生成对抗网络**（GAN）、NLP 用机器翻译；
- **降低模型复杂度**
  - 神经网络：减少网络层、神经元个数
  - 决策树：降低树的深度、剪枝
- **权值约束**（添加正则化项）：L1 正则化、L2 正则化
- **集成学习**
  - 神经网络：Dropout
  - 决策树：随机森林、GBDT
- **提前终止**
...
- 加入新特征
  - 交叉特征、多项式特征
  - 深度学习：自编码器、因子分解机、Deep-Crossing
- 增加模型复杂度
  - 线性模型：添加高次项
  - 神经网络：增加网络层数、神经元个数
- 减小正则化项的系数
  - 正则化项是为了限制模型的学习能力，减小正则化项的系数则放宽限制；
  - 模型倾向于更大的权重，更好的拟合数据

## 反向传播算法

- **梯度下降法**用损失函数对所有参数的梯度来寻找局部最小值；

  而**反向传播算法**用于加快梯度计算，用**链式法则**对每个参数求偏导；
...
- 用 4 个公式总结：
    ...
    **标量形式**：
    ...
    > 上标 `(l)` 表示网络层，`(L)` 表示输出层；下标 `j` 和 `k` 指神经元的位置；`w_jk` 表示 `l` 层的第 `j` 个神经元与`(l-1)`层第 `k` 个神经元的连接权重

- **符号说明**：
  
    - `(w,b)` 网络参数：权值和偏置
    - `z` 上一层激活值的线性组合
    - `a` 即 "activation"，每一层的激活值
- `C` 激活函数，参数为激活值`a^(L)`
...
- 目的：向网络中引入**非线性因素**；加强网络的表示能力；
  
    > [神经网络激励函数的作用是什么？有没有形象的解释？](https://www.zhihu.com/question/22334626) - 知乎 

**为什么加入非线性因素能够加强网络的表示能力？—— 万能近似定理**

- 万能近似定理：神经网络至少要有一个非线性隐藏层，则只要给予足够数量的隐藏单元，能以任意的精度来近似任何**从一个有限维空间到另一个有限维空间**的函数。
- 如果没有非线性激活，则每一层输出都是输入的**线性组合**；无论网络有多少层，结果都是线性的

  > 《深度学习》 6.4.1 万能近似性质和深度；
- 但仅**部分层是线性**是可以的，有助于**减少网络中的参数**。
...
### Dropout 策略
- Dropout 通过**参数共享**提供了一种廉价的 Bagging 集成近似—— Dropout 策略相当于集成了包括所有从基础网络除去部分单元后形成的子网络。
