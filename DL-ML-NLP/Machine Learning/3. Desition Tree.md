...
决策树的总体流程是“分而治之”，在每个中间节点寻找一个“划分”属性，相当于特征属性。
...
当大部分的样本都属于同一类，就已经做出了决策。

引入了一个概念 ---- 纯度。**信息熵越低，纯度越高**。信息熵用来度量“信息量”，如果样本的属性都一样，包含的信息就很单一；

信息熵的公式：

![](https://latex.codecogs.com/gif.latex?Ent(D)=-\sum_{k=1}^{|y|}p_klog_2p_k)

Pk表示：当前样本集中第k类样本所占的比例。
...
简单一句话：划分前的信息熵 - 划分后的信息熵。表示向提高纯度方向迈出的“步长”。
...
特征 “学历”有三个取值： “本科”，“硕士”， “博士”，

用“学历”特征对样本集D进行划分，划分后的子集如下：
...
为何非树形结构比如Adaboost、SVM、LR、Knn、KMeans之类则需要归一化？
