Java 集合/容器，两大接口： `Collection`接口，存放单一元素； `Map` 接口，存放键值对;

-  `Collection`的子接口：`List`、`Set`、`Queue`;

  ![image-20221113121329234](C:\Users\xin\AppData\Roaming\Typora\typora-user-images\image-20221113121329234.png)
