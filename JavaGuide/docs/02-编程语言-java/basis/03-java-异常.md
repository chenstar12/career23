...
- **`Exception`** : 程序本身可处理的异常，通过 `catch` 捕获。`Exception` 又分 Checked Exception (受检查异常，必须处理) 和 Unchecked Exception (不受检查异常，可不处理)。
- **`Error`** ：程序无法处理的错误 ；Error 发生时，Java 虚拟机（JVM）会线程终止；
